...
public class SaleStateByBuyer extends JPanel  {
	...
	private String[] checkZero(int[] temp) { // 매출합계 , 미수금 합계 가 '0'이면 -> '-'으로 출력 ,데이터 있으면 원본 출력
	...
	private List<Sale> getList() { 		// 고객상호명 별 선택되었는 테이블 출력 
		...
		if (!checkBox.isSelected()) {   // 체크박스가 선택되지 않았을때 
			comboBox.setEnabled(CHECK);	// 콤보박스 사용가능 
			 ...
			 Sale sale = new Sale();
			 sale.setShopName(comboBox.getSelectedItem() + ""); // 콤보박스에서 선택된 상호명 가져오기
			 list = SaleService.getInstance().selectSaleshopNameByAll(sale); // 고객상호명 별 테이블 출력 
	...
	private Object[][] getRowData(Object[][] data, Sale c, int idx) { // 각 행에 해당하는 데이터 가져오기
